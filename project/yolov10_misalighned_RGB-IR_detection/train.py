...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    # Process 0
    if rank in [-1, 0]:
        testloader, testdata = create_dataloader_rgb_ir(test_path_rgb, test_path_ir,imgsz_test, batch_size, gs, opt,  # testloader
    ...
    if rank in [-1, 0]:
        ...
        # Test best.pt
        logger.info('%g epochs completed in %.3f hours.\n' % (epoch - start_epoch + 1, (time.time() - t0) / 3600))
        if True: #opt.data.endswith('coco.yaml') and nc == 80:  # if COCO
            if best.exists():  # speed, mAP tests
                results, _, _ = test.test(opt.data,
                                          batch_size=batch_size,
                                          imgsz=imgsz_test,
                                          conf_thres=0.001,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='./models/transformer/yolov10l_fusion_add_FLIR_aligned.yaml', help='model.yaml path')
